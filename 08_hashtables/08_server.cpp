...
// The magic macro to convert an HNode pointer back into an Entry pointer [10]
#define container_of(ptr, type, member) ({                  \
    const typeof( ((type *)0)->member ) *__mptr = (ptr);    \
    (type *)( (char *)__mptr - offsetof(type, member) );})

// The structure for the key. The hashtable node is embedded here! [8]
struct Entry {
    struct HNode node;
    std::string key;
    std::string val;
...
// The global data structure for the key space [9]
static struct {
    HMap db;
} g_data;
